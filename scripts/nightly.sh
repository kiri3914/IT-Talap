#!/usr/bin/env bash
# Ночной прогон: сбор (hh + telegram) -> Postgres -> витрины -> копия сырья.
#
...
#   любой сбор упал-> загрузку всё равно пробуем: вчерашнее могло не доехать
...
# Телеграм отдельным шагом и до загрузки: форма данных другая (поток постов,
# а не снимок выдачи), падение одного не должно уносить другое.
step "сбор telegram"      "$VENV" -m ingestion.run_telegram
